...
//--------------------------------------------------------------------------------
bool ofxCvContourFinder::checkForChange( ) {
    stringstream reportStream;
    bool struckPose = false;
    newPoses.clear(); //empty it every time we checkforchange
        //check if the width of blob has changed from its previous width
    for( int i=0; i<(int)blobs.size(); i++ ) {
        int width = blobs[i].boundingRect.width;
        int yPoint = blobs[i].boundingRect.y;
        if(prevWidth.size() > 0 && prevHeight.size() > 0){

            cout << "blob# " << i << " : height " << yPoint << " prev : " << prevHeight[i] << endl;
// || yPoint - prevHeight[i] > 120 /*IF YOU SHRANK */ || yPoint - prevHeight[i] < -160 /* IF YOU GREW*/ || width - prevWidth[i] < -80 /*IF YOU GOT NARROW*/
            if(  width - prevWidth[i] > 50 /*IF YOU GOT WIDER*/ ){
                cout << "posed" << endl;
                poses.push_back(blobs[i]); //push in the new poses to add to al the others
                newPoses.push_back(blobs[i]); //record new poses
...
//                if(report){
                    ...
                    //report = false;
//                }else{
//                    //wait a little and then set it true
//                    if(ofGetElapsedTimeMillis() - timeOfLastImportantMomemt > 2000){
//                        report = true;
...
//                }
            ...
            prevWidth.push_back(width);
            prevHeight.push_back(yPoint);
        }
